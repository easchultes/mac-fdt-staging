...
SESSION: Making MAC FAIR/Article - FDT Paper (3)
DATE: 2026-06-02
URL: https://claude.ai/chat/b145da11-aefb-42b3-95f2-de5989b6c03e
CREATED_AT: P97/R97
DESCRIPTION: Consolidated working draft of the FDT paper, v1.0 — v0.5 updated with §5 v2 Trusty URI substitution (PR #5 cleanup pass), §6.4/§6.6/§6.7 revisions per Q1–Q5 literature scan calibration, new §6.8 (catalogue refinement and audit trail with Git-parallel paragraph), reference list extended
-->
...
*Working draft v1.0 — §5 v2 Trusty URIs substituted from PR #5 cleanup pass; §6.4/§6.6/§6.7 calibrated against Q1–Q5 literature scan; §6.8 added (catalogue refinement and audit trail)*

**Target venue:** *Digital Twin* (Taylor & Francis, tandfonline.com/journals/tdtw20, ISSN 2752-5783), as a follow-up to Schultes et al. 2022 [REF-FDT-Perspective] in *Frontiers in Big Data*.
...
**Changes since v0.5:** §5 Trusty URI placeholders substituted with v2 URIs from the cleanup-pass supersession registry [REF-PR5] (38 substitutions across line 276 and lines 290–339; all 38 v2 URIs verified live on the Nanopublication Server Network at consolidation time). §6.4 (Comparison with related work) expanded to include DiSSCo as the prior FDO-based DT realisation (Lannom et al. 2020; DiSSCo five-million-DOIs milestone 2025), Peters & Schindler 2023 as the industrial-FAIR-DT comparator, and a substrate-distinction discussion identifying assertion-level granularity as the distinguishing property of the np-FDO realisation; the variant-data-resources paragraph in §6.4 is recalibrated to acknowledge intermediate-granularity PID schemes (GISAID accessions, MaveDB URNs, UniProtKB_VAR identifiers) and to restate the novelty claim at the individual-property-assertion level. §6.6 (AI-assisted authoring workflow) extended with a paragraph on the regulatory salience of substrate-level provenance under the EU AI Act, with references to recent regulatory analyses and to the model-collapse result of Shumailov et al. (2024). §6.7 (Limitations and future work) closing paragraph augmented to position the three planned companion papers (generalisation, FAIR-Ready AI methodology, foundational substrate critique). §6.8 (Catalogue refinement and audit trail) added, documenting the post-initial-publication v2 refinement pass and drawing an architectural parallel with Git's content-addressed version control. Reference list extended with sixteen entries (cleanup-pass record + substrate-landscape comparators + regulatory-framing analyses + KG-DT systematic-review citations). Target venue updated from *Frontiers in Big Data* to *Digital Twin* (Taylor & Francis). No predicate, class, scientific-claim, or SPARQL changes.
...
| Anchor Trusty URI | `https://w3id.org/np/RAngbyT2iZe4xYUcPnTnDFKDaiL5-tSmoUYzF9RK3LHdE/Alpha-RBD-Variant` | `https://w3id.org/np/RADWPV5jeyK-7pxen6hMrxjP4hPNLk3EJTibcxJCInv30/Epsilon-RBD-Variant` | `https://w3id.org/np/RA8yS9sU1tq87fXelVb4Q7uwjpoDXOsoelXM5--tfu6IQ/Eta-RBD-Variant` |
...
|---|---|---|---|---|
| Alpha | late 2020 – 2021 | `https://sws.geonames.org/6269131/` | England | `https://w3id.org/np/RAHiypVLAG5QdV2Dl09EAuvXJAsaTiOIS9JwNEGSFVPSg/Alpha-Occurrence` |
| Epsilon | March 2020 | `https://sws.geonames.org/5332921/` | California, US | `https://w3id.org/np/RAScF7yrx-LR-kvrvuW58TVoc-TEjqBIwG-CV9IlvmFUk/Epsilon-Occurrence` |
| Eta | 15 December 2020 | `https://sws.geonames.org/2328926/` | Nigeria | `https://w3id.org/np/RAhhXSQQgmCfZau05NMwwYGae21iwK_qjgOs7GzDyHn9w/Eta-Occurrence` |
...
|---|---|---:|---:|---:|---|---|---|
| Alpha | ESM | 13.379 | 16494.878 | 23.674 | `https://w3id.org/np/RAf8cMF_qJFg1LAVkriSe7X1yoIz4Kg6GVaO0qrMYoaLg/Alpha-RMSD-ESM2` | `https://w3id.org/np/RATDFrbGzT9BSFCTLpQDf4HZWNpd4ReXlzvuJJ5oaTKrQ/Alpha-SASA-ESM2` | `https://w3id.org/np/RAZc567NsX-8k7XJrKAnoBcwFI92miXJcdWrcD6U7rPPE/Alpha-pLDDT-ESM2` |
| Alpha | AlphaFold II | 2.278 | 10141.237 | 93.744 | `https://w3id.org/np/RAaZJ0IuCVz8QaGyDWn10j8TBlpIifMDxqi7HzglNuz80/Alpha-RMSD-AlphaFold2` | `https://w3id.org/np/RA2aycCXga1Q6CU9uN28ZLQqAVDXUuIgPsP8QW07LD5ws/Alpha-SASA-AlphaFold2` | `https://w3id.org/np/RArrrQfjNxWFA6C4F3-QB5qJEsQOkTeivRX7_PmWWO_hQ/Alpha-pLDDT-AlphaFold2` |
| Epsilon | ESM | 12.802 | 17620.863 | 23.736 | `https://w3id.org/np/RA7EJKrmgtLHuKzT8FBNPLo-8HMgCLzBvR2Mfxpl5iJrI/Epsilon-RMSD-ESM2` | `https://w3id.org/np/RASLEpL6BamsPTgEH1FDfQ9whKu_V3-B7hHe3mVJOsuIQ/Epsilon-SASA-ESM2` | `https://w3id.org/np/RA7QLv-gPyZjD6BWOBekq7ZWKu4hVFMraGwwJOhArOeJU/Epsilon-pLDDT-ESM2` |
| Epsilon | AlphaFold II | 2.976 | 10181.969 | 94.193 | `https://w3id.org/np/RAN_Wf7LjMSofwyG0x4uhLNln8kgPnbKzl7ZHjXGKLOc4/Epsilon-RMSD-AlphaFold2` | `https://w3id.org/np/RAE9pIZSm1uZcf3AzCBLnhC6jzl5q53sHdd6xt-z4Qglk/Epsilon-SASA-AlphaFold2` | `https://w3id.org/np/RAwjLl2XsUTPoaFrv45EmHr7K-HNlN--f3-pUTxmgDNnA/Epsilon-pLDDT-AlphaFold2` |
| Eta | ESM | 2.477 | 18026.597 | 23.150 | `https://w3id.org/np/RAPh6zDXbnsMP304CGWT3OcY8ielo_GVifxC7oDsVB8HM/Eta-RMSD-ESM2` | `https://w3id.org/np/RAeGQbuaKdL82xN1MBbkzLemD7twqcJYfxZdGVUWTX8G0/Eta-SASA-ESM2` | `https://w3id.org/np/RAOevfPSd_NZh3bek1YA8M9ZWFwGFP_pTJZ4Txjw0UWz4/Eta-pLDDT-ESM2` |
| Eta | AlphaFold II | 2.720 | 10226.641 | 94.054 | `https://w3id.org/np/RAVHbFaBJ4Ezq3LpEvHlQR2y6FbyOeA6Ltk3wk-hZZTyE/Eta-RMSD-AlphaFold2` | `https://w3id.org/np/RAjsJgM13kFOaExAskq-K-7KcEf1anAAbrBz_TROBWtP0/Eta-SASA-AlphaFold2` | `https://w3id.org/np/RAyaXL_U120427BOikVY6A_eSSxWTtInmUqUHdB4W7EYk/Eta-pLDDT-AlphaFold2` |
...
|---|---:|---|
| Alpha | 0.7005 | `https://w3id.org/np/RAZtTGo_ObQHC1hN7XIdW1JYbbRCuZqUofeUM-jsWJjds/Alpha-AgMata` |
| Epsilon | 0.7195 | `https://w3id.org/np/RAMKsiqGr-f_FCX0lIrqfyilnwgS0GGpoUS7-3kQvZFJA/Epsilon-AgMata` |
| Eta | 0.6806 | `https://w3id.org/np/RARHGQesSG3Bq1t5nkw_NwPxJj8pDTSQKwABa45dQ6Cw8/Eta-AgMata` |
...
|---|---:|---:|---:|---:|---:|---:|---|
| Alpha | 9.814 | 1.042 | 10.053 | −0.133 | 0.0732 | 0.0714 | `https://w3id.org/np/RAgsEHPr-WCHfLH2RNMFCVw4BHgqP3GvnGPfV5SqEkPRY/Alpha-DMS` |
| Epsilon | 8.939 | 0.167 | 10.254 | 0.068 | 0.0730 | 0.0705 | `https://w3id.org/np/RAs9zpNLdQ2G1UQqQPktjJho7GLdsrHWAVORbxPyC_BRA/Epsilon-DMS` |
| Eta | 9.013 | 0.241 | 10.219 | 0.033 | 0.0723 | 0.0698 | `https://w3id.org/np/RAymn8p9SjkROiji7aVPDRVdD3A33Ad5uuoYPTEkaMkrA/Eta-DMS` |
...
|---|---|---|---|
| Alpha | `mac:VOC` (Variant of Concern) | 18 December 2020 | `https://w3id.org/np/RAnX2nF3Xep8CEn5GGZbRrjPQHVORHx_0tHM4cagWFVJ8/Alpha-WHO` |
| Epsilon | `mac:VOI` (Variant of Interest) | 5 March 2021 | `https://w3id.org/np/RA-uT1JVPfe5gGFAo32mUvfynHAYLlBpqgt8rUy_qhrP4/Epsilon-WHO` |
| Eta | `mac:VOI` (Variant of Interest) | 5 March 2021 | `https://w3id.org/np/RAQmYYtUl5ZE31-1yFhP9rda1KSq_TDQipwlfCZPouAHc/Eta-WHO` |
...
|---|---|---|---|---|---|
| 9 | ESM2 | `mac:ComputationalMethod` | `10.1126/science.ade2574` [REF-Lin-ESM2] | `github.com/facebookresearch/esm` | `https://w3id.org/np/RAV_9iAUAyUyNLhVQxpTaE54ko_liTnBpIarZBnkBcbgw/ESM2-Method` |
| 9 | AlphaFold 2 | `mac:ComputationalMethod` | `10.1038/s41586-021-03819-2` [REF-Jumper-AlphaFold2] | `github.com/google-deepmind/alphafold` | `https://w3id.org/np/RAPz4KzBYjUUaURNGn4pR7PpJ5VAvv43v5dYK7gyG1DXk/AlphaFold2-Method` |
| 9 | Bio2Byte AgMata | `mac:ComputationalMethod` | `10.1093/bioinformatics/btac149` [REF-Bio2Byte] | `bio2byte.be/b2btools/` | `https://w3id.org/np/RAfY9JAThg7kIl7-63YT2KdVN7y8INAyqpnow339n3tpU/AgMata-Method` |
| 10 | Bloom Lab SARS-CoV-2 RBD DMS | `mac:ExperimentalMethod` | `10.1016/j.cell.2020.08.012` [REF-Starr-2020] | `github.com/jbloomlab/SARS-CoV-2-RBD_DMS` | `https://w3id.org/np/RAWQDkXVW2tB6LEgirShh03PKJOznYQFVLx8-YXhWfnew/BloomLab-DMS-Method` |
| 11 | GISAID EpiCoV | `mac:ObservationalMethod` | `10.2807/1560-7917.ES.2017.22.13.30494` [REF-Shu-2017]; `10.46234/ccdcw2021.255` [REF-Khare-2021] | `gisaid.org` | `https://w3id.org/np/RAdSYycNtllf-CCXJ5HMMUaF-arNhDj0oGdiZyCBC08o0/GISAID-Method` |
...
*Knowledge-graph-based DT realisations.* The Universal Digital Twin and World Avatar programme of the Cambridge–CARES group [REF-Akroyd-2021; REF-Bai-2024] is the closest non-Knowlet architectural comparator. It uses OWL ontologies, computational agents, and a dynamic knowledge graph to represent chemistry and manufacturing systems, and is the only programme in the reviewed literature to attempt cross-domain interoperability at a comparable scale. A recent systematic review of ontology use in digital twins [REF-Karabulut-2023] confirms that the broader OWL/RDF-based DT family — encompassing KG-enhanced industrial DTs [REF-Sahlab-2021], built-asset DTs [REF-Ramonell-2023], software-defined-network DTs [REF-Shaik-2023], and cognitive DTs [REF-Zheng-2021] — uses knowledge graphs extensively for interoperability and reasoning, but typically as mutable agent-updated stores rather than as repositories of immutable assertions. Substrate-level differences with the present work are material: the dynamic-KG approach treats the graph as a mutable shared state updated by autonomous agents; the np-FDO substrate treats each cross-link as an immutable, separately published nanopublication carrying its own provenance and supersession chain (§6.8). The two approaches are complementary: agent-mediated mutability serves continuous-process domains; nanopublication-mediated immutability serves accreting-knowledge domains in which retrospective inspection of the claim record is itself part of the scientific use case.

*FAIR Digital Twin realisations.* Three FDO-based or FAIR-aligned digital twin realisations occupy adjacent slots to the present work, each on a different substrate. The Distributed System of Scientific Collections (DiSSCo) treats each natural-science specimen as a Digital Specimen — explicitly framed by Lannom et al. as "a machine-actionable digital twin of the physical specimen" [REF-Lannom-2020] — and has, as of mid-2025, exposed more than five million such Digital Specimens with DOI-based persistent identifiers via the DiSSCover platform [REF-DiSSCo-5M-2025]. The Biodiversity Digital Twin (BioDT) consortium published an end-to-end FAIR Digital Twin realisation in the biodiversity-modelling domain in February 2026 [REF-BioDT-2026; REF-Sustkova-2025], demonstrating ten prototype FDTs across ecological-modelling use cases and explicitly citing the Schultes 2022 FDT Perspective [REF-FDT-Perspective] as the framing reference. The substrate choice for BioDT — RO-Crate [REF-Soiland-Reyes-2022; REF-Sustkova-2025] — packages workflows, datasets, and models at the research-object granularity, with each crate carrying machine-readable metadata bundling its components. Peters and Schindler [REF-Peters-Schindler-2023] survey the conceptual application of the FAIR principles to industrial digital twins, emphasising semantic-web technologies (URIs, RDF, SPARQL) as the integration substrate without committing to a specific FDO-unit implementation. The present work's np-FDO substrate operates at finer granularity than any of these three. A single nanopublication carries a single assertion (one observation, one method declaration, one classification), and the FDT cluster is assembled by SPARQL traversal of the `mac:isObservationOf` cross-links rather than by enumeration in a crate manifest or by resolution of a DOI to an aggregated record. Substrate-level resolution behaviour also differs: DiSSCo Digital Specimens resolve through DOI infrastructure to a DataCite-style metadata payload, BioDT crates resolve to packaged research objects, and np-FDOs resolve directly to the signed RDF assertion graph on the open Nanopublication Server Network. The substrates are structurally distinct realisations of the FAIR-FDO commitment and serve different epistemic patterns: specimen-level for the curated-collection domain that DiSSCo addresses; research-object-level for the bundled-pipeline domain that biodiversity modelling exemplifies; industrial-asset-level for the conceptual framework that Peters and Schindler describe; and assertion-level for the accreting-claim domain that the present work exemplifies. The substrates are not in technical conflict — a DiSSCo Digital Specimen record or a BioDT research object could in principle be republished as a set of np-FDOs, and conversely np-FDO assertions can be aggregated for presentation in either container — but they differ in which direction the architecture flows and accordingly in which consumers, human or machine, bear the cost of decomposition. The choice among substrates is therefore largely orthogonal to the FDO commitment itself, which any of these substrates satisfies; it is a question of deployment ecology rather than of FDO conformance.

*Variant-data resources.* The variant-tracking landscape spans surveillance platforms (Nextstrain [REF-Hadfield-2018], outbreak.info [REF-Gangavarapu-2023], GISAID [REF-Shu-2017; REF-Khare-2021]), lineage nomenclature (Pango [REF-Rambaut-2020; REF-OToole-2021]), structural prediction portals, and experimental repositories (Bloom Lab DMS portal [REF-Starr-2020; REF-Starr-2022], MaveDB [REF-MaveDB; REF-MaveMD-2026]). Several of these resources publish data with persistent identifiers at intermediate granularities. GISAID issues accession identifiers at the sequence level (`EPI_ISL_*`); Pango issues lineage names at the lineage level; MaveDB issues URNs of the form `urn:mavedb:<accession>` at the score-set level, bundling thousands of variant effect measurements under a single citable identifier [REF-MaveDB; REF-MaveMD-2026]; UniProt issues `UniProtKB_VAR_*` identifiers at the curated-variant-entity level [REF-UniProt-Variants]; and journal DOIs identify the publications in which datasets appear. None of the reviewed resources, however, publishes individual property assertions — a single binding affinity measurement, a single first-detection record, a single classification decision — as separately persistently identified, individually citable objects bearing their own provenance and signature. At the knowledge-graph layer, ClinVar [REF-ClinVar], gnomAD [REF-gnomAD], the UniProt Protein Ontology Linked Open Data [REF-PRO-LOD-2020], Wikidata [REF-Waagmeester-2020], and the GA4GH Variation Annotation Specification [REF-VA-Spec-2025] together exemplify the FAIR-aligned variant-data infrastructure that has matured over the past decade — but again at the database, integration, or schema level rather than at the individual-property-assertion level. The early proposal by Patrinos et al. [REF-Patrinos-2012] that variant data should be published as nanopublications, in a microattribution framework, was not realised at scale; the present work supplies the substrate-level realisation that was missing.

*Synthesis.* The architectural slot occupied by the present work is therefore precise: a FAIR Digital Twin realised as a cluster of nanopublication-based FDOs sharing a common biomedical-variant referent, in the Knowlet sense of [REF-Mons-2019] and the FDT sense of [REF-FDT-Perspective]. The engineering-DT standards do not address this slot (different domain, different substrate, different referent class); the knowledge-graph DTs do not address it (different substrate, mutable rather than accreting); the DiSSCo, BioDT, and Peters & Schindler FAIR-aligned realisations address adjacent slots at coarser granularities (specimen-level, research-object-level, and industrial-asset-level respectively); and the variant-data resources address an adjacent slot without the cluster-as-FDT architectural pattern and without individual-property-assertion-level persistent identifiers. The present work is not the first realisation of any individual ingredient — nanopublications, FDOs, FAIR-aligned DTs, variant knowledge graphs, structural-prediction integration, classification provenance all exist independently — but is, to our knowledge, the first realisation of their combination at assertion-level granularity.
...
The reciprocal relationship between FAIR-compliant infrastructure and trustworthy AI has acquired regulatory salience in parallel with its technical articulation. The EU AI Act [REF-EU-AI-Act-2024], whose transparency and traceability requirements for high-risk AI systems take effect through 2025–2026, attaches provenance and content-identification obligations to applications that include healthcare and clinical decision support [REF-Gilbert-2024; REF-Schmidt-2024; REF-Van-Kolfschooten-2024]. Legal analysis specifically addressing digital-twin technologies under EU law [REF-Jorgensen-2025] argues that the architectural choices governing how DT provenance is recorded directly determine compliance feasibility. Independently, Shumailov et al. [REF-Shumailov-2024] demonstrate that generative models trained recursively on model-generated data converge to degenerate distributions — a result that strengthens the practical case for verifiable traceability of training-data provenance to authoritative human-curated sources. The np-FDO substrate addresses these concerns at the architectural rather than the procedural level: per-assertion ORCID-signed provenance and per-assertion immutability are properties of the publishing act itself, not properties added by downstream auditing. Whether such substrate-level provenance proves sufficient for the operational compliance standards now being developed under the EU AI Act, and how the architecture scales to the volume requirements of clinical AI applications, are open programmes of work treated in the separate methodological paper noted above.
...
The future-work programme correspondingly comprises completion and reporting of the 3705-mutation scale-up and the Bosch compute-to-lab cycle; integration with external catalogues including the Dutch National Catalogue, the European Health Data Space, and where appropriate Nextstrain and outbreak.info; instance-level FDTs of clinical and surveillance samples; agentic consumption of the FDT graph by AI agents through the Claude–NanoDash interface in development [REF-MAC-FAIR §6.4]; and cross-domain deployment of the architecture for the four referent classes listed in §6.3, beginning with metabolite FDTs in the ongoing MAC programme. Three companion papers are planned in addition. A generalisation paper will demonstrate the np-FDO FDT pattern across additional referent classes — including antimicrobial resistance phenotypes and mathematical objects — on the same architectural substrate, providing a cross-domain test of the substrate-independence of the eleven-type pattern. A methodological paper will develop the FAIR-Ready AI inverse framing introduced in §6.6 and its operational implications for the trustworthy-AI compliance landscape sketched there. A foundational paper will examine the substrate-level distinctions between native np-FDO infrastructures and retrofit approaches that wrap pre-existing aggregate categories — research objects, specimens, datasets — inherited from pre-digital scientific workflows.

---

### 6.8 Catalogue refinement and audit trail

The published catalogue underwent a documented refinement pass after its initial publication, harmonising URI conventions across the upstream FDOs (Project, two Datasets) and completing the supersession chain across all Stage 3 instances and the Stage 4 view set. The refinement was executed through the nanopublication substrate's own machinery: each refined nanopublication was minted with `npx:supersedes` linking to its predecessor, and three intermediate-form upstream nanopublications were withdrawn via `npx:retracts`. Forty-eight nanopublication operations were performed in total; the full audit trail, including the corrected intermediate state and the unrebased commit history of the refinement work, is preserved in the project repository [REF-PR5]. The episode illustrates a property of the architecture that abstract description tends to understate: refinement of a published FDO catalogue does not require deletion, rewriting of history, or breakage of existing references. Each prior version remains resolvable on the Nanopublication Server Network under its original Trusty URI, marked as superseded by a cryptographically signed assertion, with the refinement chain machine-traversable. The retrospective inspection that this enables — a feature inherited from the substrate rather than added by the present work — is itself one of the architectural properties that distinguishes nanopublication-based FDO catalogues from substrates without per-assertion immutability and per-assertion supersession semantics.

The architectural commitment to content-addressed immutability with cryptographically signed supersession is not particular to scientific knowledge representation. Git, designed in 2005 for distributed source-code management, made the same choice: content-addressed identifiers as primary references, accreting history with explicit parent-commit pointers, and optional cryptographic signing of commits. Two decades of operational evidence across millions of independent contributors demonstrate that this architecture scales. The np-FDO substrate applies the same commitments at the granularity of single scientific assertions; the catalogue-management workflow that produced the refinement pass described here is itself anchored in a Git repository, with the supersession registry, configuration history, and refinement audit trail all carried on the same content-addressed substrate at a different scale. The parallel is structural rather than coincidental: any system that handles versioned, attributed, distributable digital content under conditions of decentralised contribution converges on a similar set of architectural requirements.
...
### Added in v1.0 (substrate landscape, regulatory framing, catalogue refinement)

* [REF-PR5] — Schultes, E. (2026). *v2 catalogue cleanup pass — upstream re-mints, Stage 3 + Stage 4 instance re-mints, doc batch, registers*. Pull request #5, mac-fdt-staging repository. https://github.com/easchultes/mac-fdt-staging/pull/5
* [REF-Lannom-2020] — Lannom, L., Koureas, D., Hardisty, A. R. (2020). *FAIR Data and Services in Biodiversity Science and Geoscience*. Data Intelligence 2(1–2):122–130. https://doi.org/10.1162/dint_a_00034
* [REF-DiSSCo-5M-2025] — Distributed System of Scientific Collections (DiSSCo). (2025). *Five million DOIs*. DiSSCo announcement. https://www.dissco.eu/five-million-dois/
* [REF-Peters-Schindler-2023] — Peters, D., Schindler, S. (2023). *FAIR for digital twins*. CEAS Space Journal 16:367–374. https://doi.org/10.1007/s12567-023-00506-y
* [REF-Karabulut-2023] — Karabulut, E., Pileggi, S. F., Groth, P., Degeler, V. (2024). *Ontologies in Digital Twins: A Systematic Literature Review*. Future Generation Computer Systems. https://doi.org/10.1016/j.future.2023.12.013
* [REF-Sahlab-2021] — Sahlab, N., Kamm, S., Müller, T., Jazdi, N., Weyrich, M. (2021). *Knowledge Graphs as Enhancers of Intelligent Digital Twins*. 2021 IEEE 4th International Conference on Industrial Cyber-Physical Systems (ICPS). https://doi.org/10.1109/icps49255.2021.9468219
* [REF-Ramonell-2023] — Ramonell, C., Chacón, R., Posada, H. (2023). *Knowledge graph-based data integration system for digital twins of built assets*. Automation in Construction. https://doi.org/10.1016/j.autcon.2023.105109
* [REF-Shaik-2023] — Shaik, T. A., Hirwe, A., Tammana, P., Kataoka, K. (2023). *Building a Digital Twin Network of SDN Using Knowledge Graphs*. IEEE Access 11:63092–63106. https://doi.org/10.1109/access.2023.3288813
* [REF-Zheng-2021] — Zheng, X., Lu, J., Kiritsis, D. (2022). *The emergence of cognitive digital twin: vision, challenges and opportunities*. International Journal of Production Research 60:7610–7632. https://doi.org/10.1080/00207543.2021.2014591
* [REF-UniProt-Variants] — UniProt Consortium. *UniProt Variants* (UniProtKB_VAR identifier scheme). https://web.expasy.org/variant_pages/
* [REF-Gilbert-2024] — Gilbert, S. (2024). *The EU passes the AI Act and its implications for digital medicine are unclear*. NPJ Digital Medicine 7:74. https://doi.org/10.1038/s41746-024-01116-6
* [REF-Schmidt-2024] — Schmidt, J., et al. (2024). *Mapping the regulatory landscape for artificial intelligence in health within the European Union*. NPJ Digital Medicine 7:148. https://doi.org/10.1038/s41746-024-01221-6
* [REF-Van-Kolfschooten-2024] — van Kolfschooten, H., van Oirschot, J. (2024). *The EU Artificial Intelligence Act (2024): Implications for healthcare*. Health Policy 149:105152. https://doi.org/10.1016/j.healthpol.2024.105152
* [REF-Jorgensen-2025] — Jørgensen, T. (2025). *Digital Twins Under EU Law*. Electronics.
* [REF-Shumailov-2024] — Shumailov, I., Shumaylov, Z., Zhao, Y., Papernot, N., Anderson, R., Gal, Y. (2024). *AI models collapse when trained on recursively generated data*. Nature 631(8024):755–761. https://doi.org/10.1038/s41586-024-07566-y
...
| 17 | Trusty URI substitution from minting session | §5 tables | **Resolved (2 June 2026)**: 38 v2 URIs substituted from PR #5 supersession registry; verified live on NSN |
...
| 19 | Catalogue cleanup pass acknowledgment | §6.8 (new) | **Resolved**: §6.8 added with Git-parallel architectural framing |
| 20 | Substrate-landscape calibration (DiSSCo, Peters & Schindler, KG-DT review) | §6.4 | **Resolved**: substrate-distinction discussion added; novelty claim recalibrated at assertion-level granularity |
| 21 | Regulatory framing (EU AI Act, model collapse) | §6.6 | **Resolved**: third paragraph added on architectural compliance with EU AI Act provenance requirements |
| 22 | Three-paper sequence registration | §6.7 closing | **Resolved**: generalisation, methodological, and foundational companion papers noted |
| 23 | Target venue update | Front matter | **Resolved**: changed from *Frontiers in Big Data* to *Digital Twin* (T&F, tdtw20) |
| 24 | `[REF-MaveMD-2026]` citation key resolution | References | Pending: verify against Rubin et al. 2025 *MaveDB 2024* or confirm as distinct resource |
...
*End of consolidated working draft v1.0. Approximate word count: ~19,000–21,000 across drafted sections. Outstanding items: front-matter authorship confirmation; verification of `[REF-MaveMD-2026]` citation key; GeoNames feature ID verification for §5.2 Table 4a; any §1–§4 narrative passes triggered by reviewer feedback. v2 Trusty URIs in §5 are populated and verified live on the NSN.*
