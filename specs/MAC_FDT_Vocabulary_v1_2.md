...
4. **Term namespace confirmed.** Per item 1's resolution and Erik's confirmation on 26 May 2026, the `mac:` prefix expands to `https://w3id.org/spaces/mac/r/ontology/` (slash-terminated). Terms sit **alongside** the ontology resource in that namespace, not under it — i.e., `mac:hasWHOVariantName` expands to `https://w3id.org/spaces/mac/r/ontology/hasWHOVariantName`, and `mac:MAC-Ontology` (the ontology resource that `dct:partOf` targets) is a sibling term, also at `https://w3id.org/spaces/mac/r/ontology/MAC-Ontology`. This corrects an earlier interpretation that placed terms under `…/MAC-Ontology/{local}`; the 45 signed nanopubs were re-signed and re-test-published on 26 May 2026 to use the correct namespace, yielding new Trusty URIs recorded in `../minting/stage1-vocabulary-drafts/manifest_test.csv`.
